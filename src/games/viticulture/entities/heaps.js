...
function VineHeaps(board, config) {
    ...
    for ( const [index, value] of new Array(2).entries() ) {
        ...
        const options =  { bgURI: null, draggable: false, x: 562 + (index * 1890), y: 3000, width: 1123, height: 794, opacity: 1 };
    ...
    for ( const [index, value] of new Array(5).entries() ) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            for (let j = 0; j < count; j++) {
                const options =  { bgURI: null, x: 100 + (index * 1900) + (j * 110), y: 3500 + (i * 110), draggable: true, width: 90, height: 90, cornerRadius: 5, custom: true };
    ...
    let season = '/vine/chip/104.png';
    const options =  { bgURI: null, x: -200, y: 2400, draggable: true, width: 90, height: 90, cornerRadius: 5, custom: true };
